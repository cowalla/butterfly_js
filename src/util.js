...
function calculateEigenvalues(matrix) {
  return numeric.eig(matrix).E.x;
}

function makeButterfly(g, size, maximum) {
  var eigenvalues = [];

  for (i = 0; i < maximum + 1; i++) {
    var alpha = parseFloat(i) / parseFloat(maximum);
    eigenvalues.push(
      calculateEigenvalues(constructMatrix(g, alpha, size))
    )
  }
